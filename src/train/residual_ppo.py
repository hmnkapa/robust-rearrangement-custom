...
def main(cfg: DictConfig):
    ...
    env: gym.Env = get_rl_env(
        ...
        desk_top_yaw_target_deg=cfg.env.get("desk_top_yaw_target_deg", 180.0),
        desk_top_yaw_total_reward=cfg.env.get("desk_top_yaw_total_reward", None),
        desk_top_yaw_bonus_reward=cfg.env.get("desk_top_yaw_bonus_reward", None),
        desk_top_yaw_success_tolerance_deg=cfg.env.get(
            "desk_top_yaw_success_tolerance_deg", 10.0
        ),
    ...
    top_yaw_bonus_rewards = torch.zeros((steps_per_iteration, cfg.num_envs))
    desk_task_successes = torch.zeros((steps_per_iteration, cfg.num_envs))
    ...
    def _desk_success_from_info(info, reward):
        top_complete = info.get(
            "desk_top_yaw_complete",
            torch.zeros_like(reward, dtype=torch.bool),
        )
        if not torch.is_tensor(top_complete):
            top_complete = torch.as_tensor(top_complete, device=reward.device)
        success_mask = info.get(
            "desk_success_mask",
            torch.zeros(
                (reward.shape[0], n_parts_to_assemble),
                dtype=torch.bool,
                device=reward.device,
            ),
        )
        if not torch.is_tensor(success_mask):
            success_mask = torch.as_tensor(success_mask, device=reward.device)
        return (
            top_complete.view(-1).bool()
            & success_mask.view(reward.shape[0], -1).bool().all(dim=1)
        ).float().detach().cpu()
    ...
    while global_step < cfg.total_timesteps:
        ...
        for step in range(0, steps_per_iteration):
            ...
            top_yaw_bonus_rewards[step] = _info_reward(
                info, "desk_top_yaw_bonus", reward
            )
            if is_desk_task:
                desk_task_successes[step] = _desk_success_from_info(info, reward)
        ...
        # Calculate the success rate. Desk uses the env's staged completion signal;
        # sparse-reward tasks keep the legacy reward-count criterion.
        if is_desk_task:
            progress_units = desk_task_successes
            success_threshold = 1
        else:
            progress_units = assembly_rewards
            success_threshold = n_parts_to_assemble
        env_success = progress_units.sum(dim=0) >= success_threshold
        ...
        mean_top_yaw_bonus = top_yaw_bonus_rewards.sum(dim=0).mean().item()
        ...
        if success_rate > 0:
            ...
            success_dones = timesteps_in_success.cumsum(dim=0) >= success_threshold
        ...
        wandb.log(
            {
                ...
                "charts/top_yaw_rewards": top_yaw_rewards.sum().item(),
